...
import { CCardBody, CCardHeader, CDataTable, CCard, CBadge, CButton, CModal, CModalHeader, CModalBody, CForm, CCol, CRow, CContainer, CFormGroup, CSelect, CModalFooter, CInput, CSpinner } from '@coreui/react'
import CIcon from '@coreui/icons-react';
import axios from 'axios';
import { confirmAlert } from 'react-confirm-alert';
import 'react-confirm-alert/src/react-confirm-alert.css';
import AddSubtitlesModal from "./AddSubtitlesModal"
...
function Subtitles() {
  ...
  // for update subtitles
  const onSubmit = async () => {
    const UpdateId = geteditdata.id
    const updatesubtitlesObj = {
      name: geteditdata.upname,
      status: geteditdata.status
    }
    await axios.put(`${BaseUrl}subtitles/${UpdateId}`, updatesubtitlesObj, {
        ...
        customUI: ({ onClose }) => {
          return (
            <CModal
              ...
              onClose={onClose}
            >
              <CModalHeader closeButton> Subtitles</CModalHeader>
              <CModalBody>
              Subtitles Updated
        </CModalBody>
              <CModalFooter>
  ...
  const editmodaltoggle = async (id) => {
    ...
    const UpdateById = await axios.get(`${BaseUrl}subtitles/${id}`, {
      headers: {
  ...
  const DeleteFetchData = async (id) => {
    await axios.delete(`${BaseUrl}subtitles/${id}`, {
      headers: { "auth": localStorage.getItem("LMS_Token") }
    }).then((response)=>{
      confirmAlert({
        customUI: ({ onClose }) => {
          return (
            <CModal
              ...
              onClose={onClose}
            >
              <CModalHeader closeButton> Subtitle delete</CModalHeader>
              <CModalBody>
      ...
      customUI: ({ onClose }) => {
        return (
          <CModal
            ...
            onClose={onClose}
          >
            <CModalHeader closeButton> Delete subtitle?</CModalHeader>
            <CModalBody>
  ...
  return (<>
            ...
            'Action':
              (item) => (
                <td >
                  ...
                  <CButton color="danger" variant="outline" shape="square" size="sm" onClick={() => DeleteSubtitles(item.id)}>
    ...
    <AddSubtitlesModal show={modal} onClose={toggle} />


    <CModal show={updatemodal} onClose={editmodaltoggle} >
      <CModalHeader closeButton>Edit Subtitles</CModalHeader>
      <CModalBody>
        <CContainer fluid>
          <CRow>
            <CCol sm="12">
                ...
                <CFormGroup>
                  <CInput type="text"
                    ...
                    {errors.upname && errors.upname.type === "required" && (
                      <p className="errorMsg animate__animated animate__headShake">Subtitles is required.</p>
                    )}

                </CFormGroup>
                <CFormGroup>
                  <CSelect value={geteditdata.status} id="select nf-password" name="status" onChange={change} >
