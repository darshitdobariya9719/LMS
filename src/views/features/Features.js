...
function Features() {
  ...
  // Add modal for feture
  const [modal, setModal] = useState(false);
  // fearure update modla
  const [updatemodal, setupdatemodal] = useState(false);
  // get value for feature edit
  ...
  const onSubmit = async () => {
    ...
    const updatefeaturesObj = {
      name: geteditdata.upname,
      status: geteditdata.status
    }
    await axios.put(`${BaseUrl}features/${UpdateId}`, updatefeaturesObj, {
      headers: {
        ...
        customUI: ({ onClose }) => {
          return (
            <CModal
              ...
              onClose={onClose}
            >
              <CModalHeader closeButton> Features</CModalHeader>
              <CModalBody>
              Features Updated
        </CModalBody>
              <CModalFooter>
  ...
  const editmodaltoggle = async (id) => {
    ...
    const UpdateById = await axios.get(`${BaseUrl}features/${id}`, {
      method: 'GET',
      headers: {
  ...
  const DeleteFetchData = async (id) => {
    await axios.delete(`${BaseUrl}features/${id}`, {
      headers: {
        "auth": localStorage.getItem("LMS_Token")
      }
        ...
        customUI: ({ onClose }) => {
          return (
            <CModal
              ...
              onClose={onClose}
            >
              <CModalHeader closeButton> Features delete</CModalHeader>
              <CModalBody>
  ...
  return (<>
            ...
            'Action':
              (item) => (
                <td >
                  ...
                  <CButton color="danger" variant="outline" shape="square" size="sm" onClick={() => DeleteFeatures(item.id)}>
    ...
    <AddFeaturesModal show={modal} onClose={toggle} />


    <CModal show={updatemodal} onClose={editmodaltoggle} >
      <CModalHeader closeButton>Edit Features</CModalHeader>
      <CModalBody>
        <CContainer fluid>
          <CRow>
            <CCol sm="12">
