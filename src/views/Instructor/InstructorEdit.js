...
const Instructor_edit = () => {
  ...
  // instructor image onchange handler
  const changeimage = (e) => {
  ...
  const onSubmit = async (data) => {
    //instructor formdata for add data of instructor
    const InstructorFormData = new FormData();
    for (var key in data) {
      InstructorFormData.append(key, data[key]); // formdata doesn't take objects
    }
    if(image.name){
      InstructorFormData.append("old_img",User.image);
      InstructorFormData.append("demo_profile", image, image.name.split(" ").join(""));
    }
    InstructorFormData.append("id_group", 3);
    InstructorFormData.append("is_instructor", 1);
    await axios.put(`${BaseUrl}users/${editid.id}`, InstructorFormData, {
      headers: {
        "auth": localStorage.getItem("LMS_Token")
      },
        ...
        customUI: ({ onClose }) => {
          return (
            ...
            >
              ...
              <CModalBody>
                Instructor data uploaded successfuly
            </CModalBody>
              <CModalFooter>
          ...
          <CCardBody>
            <CForm action="" method="post" >
              ...
              <CFormGroup>
                ...
                <CInput type="text" id="nf-email" placeholder="Enter last name.." onChange={change} value={User.last_name} name="last_name" innerRef={register({ required: true })} />
...
export default Instructor_edit;
